		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					<tr valign="top">
						<td scope="row">
							<label for="def_archive_screens_all"><?php _e(
									'Show Archive link', $this->get_textdomain()
								) ?></label>
						</td>
						<td>
							<input type="checkbox" id="def_archive_screens_all" name="archive_settings[def_archive_screens_all]" value="1"
								<?php if ( isset( $options[ 'def_archive_screens_all' ] ) ) {
									checked( '1', $options[ 'def_archive_screens_all' ] );
								} ?> />
							<span class="description"><?php _e(
									'View Archive-Possibility on all post types or put behind the post types. One type per line.',
									$this->get_textdomain()
								) ?></span>
						<textarea id="def_archive_screens" name="archive_settings[def_archive_screens]" cols="80" rows="10"
							aria-required="true" class="all-options"><?php if ( isset( $options[ 'def_archive_screens' ] ) ) {
								echo $options[ 'def_archive_screens' ];
							} ?></textarea>
							<br /><span class="description"><?php _e(
									'Your install uses the following post types:', $this->get_textdomain()
								) ?><br /><code><?php echo implode( ', ', get_post_types() ); ?></code></span>
						</td>
					</tr>
				</table>
				...
				<table class="form-table">
					<tr valign="top">
						<td scope="row">
							<label for="scheduled_archiving"><?php _e(
									'Scheduled Archiving Settings', $this->get_textdomain()
								) ?></label>
						</td>
						<td>
							<input type="checkbox" id="scheduled_archiving" name="archive_settings[scheduled_archiving]" value="1"
								<?php if ( isset( $options[ 'scheduled_archiving' ] ) ) {
									checked( '1', $options[ 'scheduled_archiving' ] );
								} ?> />
							<span class="description"><?php _e(
									'Automatically archive items older than', $this->get_textdomain()
								) ?></span>
